...
if (!isset($_SESSION['id'])) {
    header("Location: dummy_Sign_in_Vendor.php");
    exit(); // Ensure script stops execution after redirect
}

// Retrieve user data based on the logged-in ID
$logged_in_id = $_SESSION['id'];

// Fetch user data using prepared statement
$sql = "SELECT * FROM vendor_user WHERE id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param('s', $logged_in_id); // Use 's' for VARCHAR
$stmt->execute();
...
$stmtCheckPayment = $db->prepare($sqlCheckPayment);
$stmtCheckPayment->bind_param('is', $logged_in_id, $name);
...
$stmtCheckPaymentConfirmation = $db->prepare($sqlCheckPaymentConfirmation);
$stmtCheckPaymentConfirmation->bind_param('is', $logged_in_id, $name);
...
if (isset($_POST['pay']) && $paymentStatus === "To be paid") {
    ...
    $stmtInsertPayment = $db->prepare($sqlInsertPayment);
    $stmtInsertPayment->bind_param('ssds', $logged_in_id, $name, $balance, $paymentDate); // Use 's' for VARCHAR and 'd' for DOUBLE
...
<body>
    ...
    <br>
    <a href="Vendor_Messages.php"><button>Chat</button></a>
</body>
</html>